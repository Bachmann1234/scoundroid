...
/**
 * A player that uses the heuristic strategy but records all decisions
 * for neural network training.
 */
class DataCollectionPlayer(
    private val genome: PlayerGenome = PlayerGenome.DEFAULT,
) {
    private val decisions = mutableListOf<PendingDecision>()

    data class PendingDecision(
        val features: GameStateFeatures,
        val roomCards: List<String>,
        val decision: Decision,
    )

    fun playGameAndCollect(initialState: GameState): Pair<GameState, List<TrainingExample>> {
        decisions.clear()
        ...
        val gameWon = isActuallyWon(state)
        val finalScore = state.calculateScore()

        // Convert pending decisions to training examples with outcome
        val examples = decisions.map { pending ->
            TrainingExample(
                features = pending.features,
                roomCards = pending.roomCards,
    ...
    private fun playOneStep(state: GameState): GameState {
        ...
        if (room.size == GameState.ROOM_SIZE) {
            // Record the decision
            val features = GameStateFeatures.fromGameState(state)
            if (features != null) {
                val roomCards = room.map { "${it.rank.displayName}${it.suit.symbol}" }

                if (!state.lastRoomAvoided && shouldSkipRoom(state, room)) {
                    decisions.add(PendingDecision(features, roomCards, Decision.SkipRoom))
                    return state.avoidRoom()
                }

                val cardToLeave = chooseCardToLeave(state, room)
                val cardIndex = room.indexOf(cardToLeave)
                decisions.add(PendingDecision(features, roomCards, Decision.LeaveCard(cardIndex)))
            }
    ...
    private fun processCard(state: GameState, card: Card): GameState = when (card.type) {
    ...
    private fun processCombat(state: GameState, monster: Card): GameState {
        ...
        val shouldUse = when {
            ...
            else -> damageSaved >= genome.minDamageSavedToUseWeapon
        }
