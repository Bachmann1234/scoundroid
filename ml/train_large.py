#!/usr/bin/env python3
"""
Training with a LARGER network architecture.
"""
...
class LargeActorCritic(nn.Module):
    """Larger network: 1024 hidden units, more layers."""
    def __init__(self, input_size=26, hidden_size=1024):
        super().__init__()
        ...
        self.actor = nn.Linear(hidden_size // 8, 5)
        self.critic = nn.Linear(hidden_size // 8, 1)
...
def evaluate(model: LargeActorCritic, num_episodes: int = 1000):
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Large Network Training')
    ...
    parser.add_argument('--lr', type=float, default=0.00005)  # Lower LR for larger network
    ...
    parser.add_argument('--output', type=str, default='rl_large.pt')
    ...
    print(f"Network: 1024 hidden units (2x normal)")
    print(f"Episodes: {args.episodes:,}")
    print(f"Learning rate: {args.lr}")
    print(f"Batch size: {args.batch_size}")
    print()

    model = LargeActorCritic()
